...
p1 <- ggplot(data1, aes(x = Year, y = Rating, color = Entity)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
...
# Example accurate ratings (adjust with actual data)
# Updated accurate ratings for Stockfish, Leela Chess Zero, and Komodo
data2 <- data.frame(
  Year = rep(2018:2023, 3),
  Engine = rep(c("Stockfish", "Leela Chess Zero", "Komodo"), each = 6),
  Rating = c(
    # Ratings for Stockfish
    3520, 3540, 3560, 3580, 3600, 3620,
    # Ratings for Leela Chess Zero
...
p2 <- ggplot(data2, aes(x = Year, y = Rating, color = Engine)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
...
Well, this is interesting! Leela Chess Zero is a hundred or so rating points above Stockfish. Why is this the case? Well, Leela Chess Zero is designed with a method called "reinforcment learning." This is a machine learning algorithm, which has been used in Google's Deep Mind AlphaZero. Stockfish uses the "minimax" algorithm, which considers a set of potential stratigies and chooses the best one, but is not close to the depth of Leela Chess Zero.
